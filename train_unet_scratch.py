...
def train_unet():
    ...
    print("Launching Clean Multi-Class UNet (From Scratch) Training")
    
    # --- ISOLATED DIRECTORY SETUP ---
    out_dir = "outputs/unet_scratch/"
    ...
    # --- Explicitly track 3 images for deliverables ---
    tracked_images = val_names[:3]

    train_ds = SofaComponentDataset(
        image_dir=train_input_dir,
        mask_dir=train_gt_dir,
        image_names=train_names,
        transform=get_unet_train_transforms()
    )
    ...
    # Kept the 1e-2 weight decay we established for the scratch model earlier
    ...
    for epoch in range(epochs):
        ...
        for imgs, masks, _ in loop:
            ...
            # Standard U-Net specific padding
            imgs_padded = F.pad(imgs, (92, 92, 92, 92), mode='reflect')
            
            optimizer.zero_grad()
            with torch.amp.autocast('cuda'):
                out = model(imgs_padded) 
        ...
        if avg_v_miou > best_val_miou:
            ...
            torch.save(model.state_dict(), os.path.join(out_dir, "best_unet_scratch.pth"))
            print(f"New UNet Record: {best_val_miou:.4f}")
